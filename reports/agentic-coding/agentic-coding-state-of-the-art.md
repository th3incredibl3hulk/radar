...
The 2026-07-28 release candidate (final spec ships that date — now inside its last week) is the largest MCP revision since launch: a stateless core (no `initialize`/`initialized` handshake, no `Mcp-Session-Id`, any request routable to any server instance behind a plain load balancer), two new versioned extensions (MCP Apps for sandboxed server-rendered UI; Tasks for stateless long-running work via `tasks/get`/`update`/`cancel`, demoted out of core spec into an extension after production use on the 2025-11-25 experimental version surfaced redesign pressure — `tasks/list` is removed entirely since it can't be scoped safely without sticky sessions), tightened OAuth/OIDC alignment (six SEPs, `iss` validation per RFC 9207), and the protocol's first formal deprecation policy (Roots, Sampling, Logging deprecated with a 12-month minimum removal window). Reference implementation risk is being managed tightly: the Python SDK's stable v2.0 is targeted for July 27, one day ahead of the spec itself.
