...
As of this cycle (2026-07-20), a third theme has joined those two: benchmark credibility is degrading almost as fast as new benchmarks ship. OpenAI retracted its own recommendation of SWE-Bench Pro after finding ~30% of its public tasks broken — the second major SWE-bench-family credibility hit in 2026 (after SWE-bench Verified's deprecation for contamination in February). Any vendor comparison built on recent SWE-bench Pro numbers should be treated as provisional. Separately, open-weight models closed real ground this cycle: Moonshot AI's 2.8T-parameter Kimi K3 now tops the Frontend Code Arena and trails GPT-5.6 Sol by half a point on Terminal-Bench 2.1 — the strongest open-weight coding result to date.
